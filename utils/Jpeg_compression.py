...
class JpegCompression(nn.Module):
    def __init__(self, device, yuv_keep_weights=(9, 9, 25)):
    ...
    def forward(self, images):
        ...
        image_process = images.clone()

        # pad the image so that we can do dct on 8x8 blocks
        pad_height = (8 - image_process.shape[2] % 8) % 8
        pad_width = (8 - image_process.shape[3] % 8) % 8
        image_process = nn.ZeroPad2d((0, pad_width, 0, pad_height))(image_process)

        assert image_process.shape[2] % 8 == 0
        assert image_process.shape[3] % 8 == 0

        # apply dct
        image_dct = self.apply_conv(image_process, 'dct')
