...
_rafswap_dir = os.path.join(_current_dir, 'RAFSwap')
if _rafswap_dir not in sys.path:
    sys.path.insert(0, _rafswap_dir)
...
def _get_parser(device):
    global _parser
    if _parser is None:
        ckpt = os.path.join(_rafswap_dir, 'pretrained_models', '79999_iter.pth')
...
def composite_face(original, fake, fluent=0):
    mask = get_face_mask(original)  # [B, 1, H, W]
