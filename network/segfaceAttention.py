...
class SegFaceWatermarkAdapter:
    def __init__(self, device="cuda"):
        ...
        self.watermark_regions = {
            "background": {"index": 0, "weight": 1.0},   # 新增
            "face": {"index": 2, "weight": 1.1},       # 面部
            "lefteye": {"index": 9, "weight": 1.3},    # 左眼
            "righteye": {"index": 8, "weight": 1.3},   # 右眼
            "nose": {"index": 10, "weight": 1.3},      # 鼻子
            "mouth": {"index": 11, "weight": 1.3},     # 口腔
            "leftbro": {"index": 7, "weight": 1.1},    # 左眉
            "rightbro": {"index": 6, "weight": 1.1}    # 右眉
        }

    def _load_model(self):
        model = SegFaceCeleb(512, "mobilenet").to(self.device)

        local_model_path = "/root/segfacemark_v/model/model_299.pt"
