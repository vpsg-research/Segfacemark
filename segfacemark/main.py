...
from pytorch_ssim import ssim as ssim_loss_fn
...
class IWNet(nn.Module):
    ...
    def fit(self, log_dir=False, batch_size=cfg.batch_size, lr=float(cfg.lr), epochs=cfg.epochs, resume_from=None):
        if not log_dir:
        ...
        start_epoch = 1
        optimizer_encoder = optim.Adam(self.encoder.parameters(), lr=lr, weight_decay=1e-5)
        optimizer_decoder = optim.Adam(self.decoder.parameters(), lr=lr, weight_decay=1e-5)

        if resume_from is not None:
            print(f"从 checkpoint 续训: {resume_from}")
            ckpt = torch.load(resume_from, map_location=cfg.device)
            self.load_state_dict(ckpt.get("model_state", ckpt), strict=True)
            if "opt_encoder" in ckpt:
                optimizer_encoder.load_state_dict(ckpt["opt_encoder"])
            if "opt_decoder" in ckpt:
                optimizer_decoder.load_state_dict(ckpt["opt_decoder"])
            start_epoch = ckpt.get("epoch", 0) + 1
            print(f"模型+优化器已恢复，从 epoch {start_epoch} 继续训练")

        with open(os.path.join(log_dir, "config.json"), "wt") as out:
            out.write(json.dumps(dict(cfg), indent=2, default=lambda o: str(o)))
        ...
        for epoch in range(start_epoch, epochs + 1):
            ...
            print(f"Epoch {epoch} - 训练阶段，解码器PCA路径：{self.decoder.pca_save_path}")
            iterator = tqdm(train)
            cur_lr = 0.0
            ...
            for step, (cover_images, wms, mask) in enumerate(iterator):
                ...
                face_mask = torch.zeros_like(masks_scaled, dtype=torch.bool)
                for idx in face_indices:
                    face_mask = face_mask | (masks_scaled == idx)
                face_mask = face_mask.float().unsqueeze(1)   # [B, 1, H, W]
                bg_mask = 1.0 - face_mask                     # [B, 1, H, W]
                ...
                extract_wm_all, regional_wms, region_masks = decode(b_embedded_attack_type_all, self.decoder, masks_scaled)
                ori_regional_wms = []
                ...
                diff = B - b_embedded
                loss_encoder_face = torch.mean(diff * diff * face_mask) * cfg.encoder_w_face
                loss_encoder_bg   = torch.mean(diff * diff * bg_mask)   * cfg.encoder_w_bg
                loss_encoder = loss_encoder_face + loss_encoder_bg

                loss_noise_all = mse(extract_wm_all, watermark)
                loss_rgb = mse(watermarked_images, cover_images) * cfg.rgb_w
                # SSIM期望输入[0,1]，需从[-1,1]转换
                wm_rgb_01 = watermarked_images * 0.5 + 0.5
                cv_rgb_01 = cover_images * 0.5 + 0.5
                loss_ssim = (1.0 - ssim_loss_fn(wm_rgb_01, cv_rgb_01)) * cfg.ssim_w
                loss_total = loss_encoder + loss_noise_all * cfg.all_w + loss_gnn * cfg.gnn + loss_rgb + loss_ssim
            ...
            print(f"Epoch {epoch} - 测试阶段，解码器PCA路径：{self.decoder.pca_save_path}")
            ...
            with torch.no_grad():
                for step, (images, wms, mask) in enumerate(iterator):
                    ...
                    val_identity_wm_all, regional_wms_identity, _ = validation_attack(input, b_embedded, identity, self.decoder, masks_scaled)
                    val_stargan_wm_all, regional_wms_stargan, _ = validation_attack(input, b_embedded, stargan, self.decoder, masks_scaled, type="all")
                    val_ganimation_wm_all, regional_wms_ganimation, _ = validation_attack(input, b_embedded, ganimation, self.decoder, masks_scaled, type="all")
                    val_cscs_wm_all, regional_wms_cscs, _ = validation_attack(input, b_embedded, cscs, self.decoder, masks_scaled, type="all")
                    val_hififace_wm_all, regional_wms_hififace, _ = validation_attack(input, b_embedded, hififace, self.decoder, masks_scaled, type="all")
                    val_RAFSwap_wm_all, regional_wms_RAFSwap, _ = validation_attack(input, b_embedded, RAFSwap, self.decoder, masks_scaled, type="all")
                    val_infoswap_wm_all, regional_wms_infoswap, _ = validation_attack(input, b_embedded, infoswap, self.decoder, masks_scaled, type="all")
                    val_unifaceswap_wm_all, regional_wms_unifaceswap, _ = validation_attack(input, b_embedded, uniface_swap, self.decoder, masks_scaled, type="all")
                    val_fsrt_wm_all, regional_wms_fsrt, _ = validation_attack(input, b_embedded, fsrt, self.decoder, masks_scaled, type="all")
                    # val_stylemask_wm_all, regional_wms_stylemask, _ = validation_attack(input, b_embedded, stylemask, self.decoder, masks_scaled, type="all")
            ...
            checkpoint = {
                ...
                "opt_encoder": optimizer_encoder.state_dict(),
                "opt_decoder": optimizer_decoder.state_dict(),
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 断点续训路径在 config/train.yaml 的 resume_from 字段，留空则从头训练
        resume_path = getattr(cfg, "resume_from", None)
        model.fit(resume_from=resume_path if resume_path else None)
